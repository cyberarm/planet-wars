class GameUpgradeHUD
  def initialize(ship)
    ...
    @upgrade_speed_text  = Text.new('', x: $window.width-(30*10), y: 450, size: 18)
    @upgrade_boost_text  = Text.new('', x: $window.width-(30*10), y: 470, size: 18)
    @upgrade_boosc_text  = Text.new('', x: $window.width-(30*10), y: 490, size: 18)
  end
  ...
  def update
    ...
    @upgrade_boosc_text.text = "Capacity: #{@ship.max_boost}/#{@ship.lock_max_boost}"
  end
end
